...
import pandas as pd # Pandas for lv management
from datetime import date # Library to deal with date objects
from datetime import timedelta # Library to deal with differences between 2 dates (for dates between today and expected collection dates)
import itertools

# Connect to SQL server to get lv


# ATTN: Below is the connection string. The server address (SERVER), database (DATABASE), user ID (UID), and TRUSTED_CONNECTION has been removed. Please type them back in in the <> (remove <> too)
# , and run this script on a schedule
################### EDIT ME! ############################
...
query2='''SELECT a.[RHO Screening Identifier], a.[Participant ID]
              , a.[Cohort], a.[Screening Date], a.[Enrollment Date], a.[Complete Date], a.[Terminate Date], a.[Terminate Reason], a.[Participant Status]
              , d.[Site]
              , b.[Activity], b.[Visit Date]
              , c.[Visit Number], c.[Visit Ordinal], c.[Visit Description], c.[VisitCategory]
...
rho["Participant ID"]=rho.apply(lambda x: pid_from_screening_identifier(x["RHO Screening Identifier"]),axis=1)
...
# %%
A_visit_names=['0A', '1A', '2A', '3A', '4A', '7A', '10A', '13A', '17A', '18A', '19A', '20A', '21A', '22A', '23A', '24A', '25A']
A_visit_weeks=[0,1,2,3,4,12,24,36,52,65,78,91,104,117,130,143,156]
B_visit_names=['0B', '1B', '3B', '5B', '7B', '9B', '10B', '11B', '12B', '13B', '14B', '15B', '16B', '17B']
...
for participant in unique_pids: # For every participant that has a mechanistic visit...
    ...
    participant_visits_rho=list(rho_pid_with_mech_visit[rho_pid_with_mech_visit["Participant ID"]==participant]["Visit Number"].unique())
    ...
    participant_cohort=data_for_participant[data_for_participant["Participant"]==participant]["Cohort"].unique()[0]

    #If the cohort is A:
    if participant_cohort=="A":

        # Set the list of expected visits for that participant to be the list of visits expected for cohort A, without DV
    ...
    #If the cohort is B:
        ...
        # Set the list of expected visits for that participant to be the list of visits expected for cohort B, without DV
        expected_visits_for_pid=b_vis
        if "DVB" in expected_visits_for_pid:
            expected_visits_for_pid.remove("DVB")
    for visit in expected_visits_for_pid: #For every visit we could possibly expect from a participant:
        for specimen in specimen_type: # For each specimen type we can expect:
            count=len(data_for_participant[(data_for_participant["visitnum"]==visit) & (data_for_participant["specimentype"]==specimen)])
            if visit=="0A" or visit=="0B":
                expected_date=data_for_participant[(data_for_participant["visitnum"]==visit)]["CollectionDate"].unique()[0]
                #print(participant,visit,specimen,count,expected_date)
                append_list=[participant,visit,specimen,count,expected_date,participant_cohort,site_code_for_participant]
                zero_df.loc[len(zero_df)] = append_list
            else:
                visit_0="0"+str(participant_cohort)
                expected_date_v0=data_for_participant[(data_for_participant["visitnum"]==visit_0)]["CollectionDate"].unique()[0]
                if participant_cohort=="A":
    ...
    if has_DV:
        ...
        expected_date_dv=data_for_participant[(data_for_participant["visitnum"]==dv_visit_name)]["CollectionDate"].unique()
        if len(expected_date_dv)==0:
            expected_date_dv=rho[(rho["Participant ID"]==participant) & (rho["Visit Number"]==dv_visit_name)]["Visit Date"].unique()[0]
        else:
            expected_date_dv=expected_date_dv[0]
        for specimen in specimen_type:
            
            count=len(data_for_participant[(data_for_participant["visitnum"]==dv_visit_name) & (data_for_participant["specimentype"]==specimen)])
            
            append_list=[participant,dv_visit_name,specimen,count,expected_date_dv,participant_cohort,site_code_for_participant]
            zero_df.loc[len(zero_df)] = append_list
...
for i in list(zero_df["PID"].unique()):
    ...
    if not (("DVA" in visits_for_pid) or ("DVB" in visits_for_pid)):
        ...
        dv_expected_date=list(zero_df[(zero_df["PID"]==i) & (zero_df["visitnum"]==dv_visit)]["expectDate"].unique())[0]
        #print(dv_expected_date)
        zero_df.drop(zero_df[(zero_df["PID"]==i) & (zero_df["expectDate"]>=dv_expected_date)].index,inplace=True)
...
zero_df.drop(zero_df[ (zero_df["visitnum"].isin(['1A','2A','3A'])) & (zero_df["cohort"]=='A') & (zero_df["specimentype"].isin(["Urine Pellet","Urine Super","Whole Blood"]))].index,inplace=True)
...
def count_alert(specimen,visit,count,participant):
    if specimen=="Serum-Clot":
        if visit == "0A":
            if count > 20:
                return "More {} tubes collected than expected for PID {} at visit {}!".format(specimen,participant,visit)
            elif count < 20:
                return "Less {} tubes collected than expected for PID {} at visit {}!".format(specimen,participant,visit)
        else:
            if count > 23:
                return "More {} tubes collected than expected for PID {} at visit {}!".format(specimen,participant,visit)
            elif count < 23:
...
zero_df["alert"]=zero_df.apply(lambda x: count_alert(x["specimentype"],x["visitnum"],x['count'],x["PID"]),axis=1)
...
# %%
zero_df[(zero_df["PID"]=='10368') & (zero_df["visitnum"]=='13A') & (zero_df['specimentype']=="Urine Pellet")]["count"].values[0]
...
def mssg(s_ab,cde,ID,spec,vis,viswk,tdy,edate):
    ...
    return '''ITN080AI REBOOT - {}/{} - PID {}; {} specimens @ Visit {} (Week {}) {} days overdue for collection (from expected visit)'''.format(s_ab,cde,ID,spec,vis,viswk,dte)

# Apply the message function
if len(zero_df) != 0:
    zero_df["MSSG"]=zero_df.apply(lambda x: mssg(site_abb[x["site"]],x["site"],x["PID"],x["specimentype"],x["visitnum"],all_visit_to_week[x["visitnum"]],date.today(),x["expectDate"]),axis=1)
...
insert_string='''INSERT INTO [DAVE].[input].[pythonExpectedCollected] ([Study Number],[Participant ID],[Visit Number],[Specimen Type],[Visit Date],[STS Cohort],[Site Code],[Days Overdue],[Message]) VALUES (?,?,?,?,?,?,?,?,?)'''
tuples=[(i[10],i[0],i[1],i[2],i[4],i[5],i[6],i[8],i[9]) for i in zero_df.values.tolist()]

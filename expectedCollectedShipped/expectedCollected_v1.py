...
if __name__ == "__main__":
    ...
    for participant in unique_pids:
        ...
        participant_visits_rho = list(
            rho_pid_with_mech_visit[rho_pid_with_mech_visit["Participant ID"] == participant]["Visit Number"].unique()
        )
        ...
        for visit in expected_visits_for_pid:
            ...
            for specimen in specimen_type:
                ...
                if visit == "0A" or visit == "0B":
                    expected_date = \
                        data_for_participant[(data_for_participant["visitnum"] == visit)]["CollectionDate"].unique()[0]
        ...
        if specimen == "Serum-Clot":
            if visit == "0A":
                if count > 20:
                    ...
                    return "Less {} tubes collected than expected for PID {} at visit {}!".format(specimen,
                                                                                                  participant,
                                                                                                  visit)
            else:
                if count > 23:
    ...
    if len(zero_df) != 0:
        zero_df["MSSG"] = zero_df.apply(lambda x: mssg(site_abb[x["site"]],
                                                       x["site"], x["PID"], x["specimentype"],
                                                       x["visitnum"], all_visit_to_week[x["visitnum"]],
                                                       date.today(), x["expectDate"]), axis=1
                                        )
